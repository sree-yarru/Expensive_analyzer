"""
Email Alectra Utilities Agent
==============================
Scans your email inbox for Alectra Utilities (Brampton) bill notifications,
...
ALECTRA_SENDER = os.environ.get("ALECTRA_SENDER", "")
ALECTRA_START_DATE = os.environ.get("ALECTRA_START_DATE", "2024-01-01")
ALECTRA_END_DATE = os.environ.get("ALECTRA_END_DATE", "2026-03-18")

# Google Sheet settings
ALECTRA_SHEET_URL = os.environ.get("ALECTRA_SHEET_URL", os.environ.get("SHEET_URL", ""))
ALECTRA_TAB_SUMMARY = os.environ.get("ALECTRA_TAB_SUMMARY", "Alectra_Bills")
ALECTRA_TAB_TRACKER = os.environ.get("ALECTRA_TAB_TRACKER", "alectra_tracker")

# Extraction config file
ALECTRA_CONFIG_FILE = os.environ.get("ALECTRA_EXTRACTION_CONFIG", "extraction_config_alectra.json")
...
CONFIG = load_extraction_config(ALECTRA_CONFIG_FILE)
...
def extract_with_gemini(text: str, api_key: str, config: dict) -> dict:
    ...
    prompt = f"""You are an expert data extraction assistant specializing in utility bills.
Extract all billing information from the following Alectra Utilities email or bill text.
...
- For kWh, return the number only.
...
def append_bill_to_sheet(processed_data: dict, email_subject: str, email_date,
                         ...
                         config: dict):
    """Write extracted Alectra bill summary data to Google Sheets (summary only)."""
    gc = gspread.service_account(filename=creds_path)
...
def main():
    print("=" * 60)
    print("⚡ Alectra Utilities Bill Agent")
    print("=" * 60)
    ...
    if not ALECTRA_SHEET_URL:
        print("❌ Error: ALECTRA_SHEET_URL (or SHEET_URL) must be set in .env")
        return

    # Parse senders (support comma-separated list)
    senders = [s.strip() for s in ALECTRA_SENDER.split(",") if s.strip()]

    # Parse dates
    try:
        start_date = datetime.strptime(ALECTRA_START_DATE, "%Y-%m-%d").date()
        end_date = datetime.strptime(ALECTRA_END_DATE, "%Y-%m-%d").date()
    except Exception as e:
    ...
    print(f"📊 Sheet:    {ALECTRA_SHEET_URL[:60]}...")
    print(f"📋 Tab:      {ALECTRA_TAB_SUMMARY} (summary only)")
    print(f"⚙️  Config:   {ALECTRA_CONFIG_FILE}")
    print("-" * 60)
    ...
    try:
        with MailBox(EMAIL_HOST).login(EMAIL_USER, EMAIL_PASS) as mailbox:
            ...
            for idx, msg in enumerate(all_messages, 1):
                ...
                if check_if_processed(email_id, CREDS_FILE, ALECTRA_SHEET_URL, ALECTRA_TAB_TRACKER):
                ...
                try:
                    ...
                    append_bill_to_sheet(
                        ...
                        CREDS_FILE, ALECTRA_SHEET_URL,
                        ALECTRA_TAB_SUMMARY, ALECTRA_TAB_TRACKER,
                        CONFIG
                    )
                    ...
                    kwh = processed_data.get("total_kwh", "?")
                    print(f"   ✅ Bill #{bill_num} | ${amount} due {due} | {kwh} kWh")
                    processed_count += 1
